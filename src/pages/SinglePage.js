				...
				{movie && !loading && <>
					<Grid>
						...
						<MovieDetails>
							...
							<Text>Rating: <Rating highlyRated={movie.vote_average >= 7} >{movie.vote_average.toFixed(1)}</Rating></Text>

							{actors && <Carousel list={actors.slice(0, 9)} />}

							<Similar><strong>Similar :</strong> {similar.map(s => <SimilarLink key={s.id} onClick={() => handleSimilar(s.id)} as="span" > {s.title || s.name}, </SimilarLink>)}</Similar>
